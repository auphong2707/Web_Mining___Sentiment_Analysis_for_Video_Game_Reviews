...
The first step involved identifying a diverse list of games to ensure the dataset covers various genres, platforms, and time periods. We implemented a custom Python script, \texttt{discover\_games.py}, which employs standard HTTP requests and BeautifulSoup for HTML parsing. This script iterates through Metacritic's game listings, utilizing pagination handling to traverse multiple pages of search results, extracting key metadata for each game:
\begin{itemize}
    \item \textbf{Game Title:} The canonical name of the video game.
    \item \textbf{Platform:} The specific console or system (e.g., PlayStation 5, PC, Xbox Series X) to account for performance differences that might affect sentiment.
    \item \textbf{Metascore:} The aggregate critic score, collected for dataset completeness.
    \item \textbf{Release Date:} The game's original release date for temporal analysis.
    \item \textbf{URL:} The direct link to the game's user review page.
\end{itemize}
The output of this module is a structured list of game URLs stored in \texttt{data/discovered\_games/}, serving as the input queue for the review scraper.

\subsubsection{Review Scraping Engine}
To extract the actual review content, we developed a sophisticated scraping engine using \textbf{Scrapy}, a high-level web crawling framework, integrated with \textbf{Playwright}. This combination was essential because Metacritic relies heavily on JavaScript to render user reviews dynamically. Standard HTTP requests (used by libraries like \texttt{requests} or \texttt{BeautifulSoup}) would fail to load the full content.

The spider, located in \texttt{metacritic\_scraper/spiders/}, operates as follows:
\begin{enumerate}
    \item \textbf{Request Handling:} It accepts the list of discovered game URLs and initiates asynchronous requests with configurable concurrency (\texttt{CONCURRENT\_REQUESTS = 2-4}).
    ...
    \item \textbf{Platform Discovery:} The spider automatically detects all available platforms for multi-platform games by parsing the platform filter dropdown, then scrapes reviews for each platform separately to ensure comprehensive coverage.
    \item \textbf{Progressive Loading:} To load all available reviews, Playwright automatically scrolls through the page and clicks "Load More" buttons, with timeout protection (300 seconds per platform) and duplicate detection to prevent infinite loops.
    ...
    \begin{itemize}
        ...
        \item \textbf{Review Platform:} The specific platform for which the review was written, extracted at the individual review level.
    ...
    \end{itemize}
    \item \textbf{Rate Limiting:} To adhere to ethical scraping guidelines and avoid IP bans, we configured \texttt{DOWNLOAD\_DELAY = 3} seconds, enabled auto-throttling (\texttt{AUTOTHROTTLE\_ENABLED = True}), and limited concurrent requests in both the spider's custom settings and \texttt{settings.py}, ensuring a respectful request pace that adapts to server response times.
\end{enumerate}
The raw output is saved as JSON Lines (\texttt{.jsonl}) files in the \texttt{data/review\_data/} directory, with each review stored as a separate JSON object on its own line, facilitating efficient line-by-line processing in subsequent stages.
...
We developed an aggregation pipeline (\texttt{aggregate\_dataset.py}) to consolidate the scattered JSONL files from Phase 1. Key preprocessing steps included:
\begin{itemize}
    ...
    \item \textbf{Deduplication:} Duplicate reviews (e.g., spam or multiple scrapes of the same page) were identified via normalized text comparison and removed to prevent data leakage between training and test sets. The system tracks unique reviews using lowercase, trimmed text as a hash key.
...
To guarantee rigorous evaluation, the processed dataset was deterministically split using a fixed random seed (\texttt{random.seed(42)}) into three subsets:
\begin{itemize}
...
\end{itemize}
The splitting was performed after shuffling to ensure random distribution across all sentiment categories and prevent temporal or game-specific biases. The final dataset was formatted as a HuggingFace \texttt{DatasetDict} and uploaded to the HuggingFace Hub using \texttt{prepare\_and\_upload\_hf\_dataset.py}, enabling seamless integration with modern NLP libraries such as Transformers and Datasets.
...
Using the best learning rate $\eta^*$ identified in Stage 1, we train on the complete dataset (100\% of training data):
\begin{itemize}
    \item \textbf{Validation Frequency:} Every 500 training steps (standard interval for full-scale training).
    \item \textbf{Checkpoint Management:} When validation F1 improves, a checkpoint is saved containing model state, optimizer state, and performance metrics. Only the top 2 checkpoints are retained to conserve storage.
    \item \textbf{Model Selection:} After training completes, the checkpoint with the highest validation F1 score is automatically loaded for final test set evaluation, serving as an implicit early stopping mechanism.
\end{itemize}
...
\textbf{Stage 2: Final Training with Optimal Configuration.}
Using the best learning rate $\eta^*$ identified in Stage 1, we fine-tune on the complete dataset (100\% of training data):
\begin{itemize}
    \item \textbf{Validation Frequency:} Every 1000 training steps (standard interval for full-scale training).
    \item \textbf{Checkpoint Management:} When validation F1 improves, a checkpoint is saved containing model state, optimizer state, and performance metrics. Only the top 2 checkpoints are retained to conserve storage.
    \item \textbf{Model Selection:} After training completes, the checkpoint with the highest validation F1 score is automatically loaded for final test set evaluation, serving as an implicit early stopping mechanism.
\end{itemize}
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Fixed Parameters:} Number of estimators fixed at 5000 with early stopping to find optimal iteration count.
...
\begin{itemize}
    \item \textbf{Training Protocol:} XGBoost's built-in early stopping monitors validation performance every boosting round, terminating training when validation loss fails to improve for 200 consecutive rounds.
    \item \textbf{Model Selection:} The model at the iteration with lowest validation loss is automatically retained for final evaluation.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Positive reviews:} 368{,}669 samples (58.5\%)
    \item \textbf{Negative reviews:} 159{,}847 samples (25.4\%)
    \item \textbf{Mixed reviews:} 101{,}368 samples (16.1\%)
\end{itemize}
...
The high standard deviation of 766 characters indicates pronounced heterogeneity in review verbosity. The dataset includes extremely brief comments as well as extensive, essay-style critiques. This variability reflects diverse user writing styles and necessitates models capable of handling both concise expressions and long-form discourse.
...
\begin{itemize}
    \item \textit{Quality indicators:} ``great'' (150{,}546), ``best'' (122{,}688), ``amazing'' (68{,}706), ``good'' (144{,}757)
    \item \textit{Enjoyment markers:} ``fun'' (120{,}103), ``love'' (frequent)
    \item \textit{Narrative elements:} ``story'' (164{,}517), ``characters'' (65{,}135), ``world'' (69{,}870)
    \item \textit{Technical aspects:} ``gameplay'' (94{,}128), ``graphics'' (75{,}759)
\end{itemize}
...
\begin{itemize}
    \item \textit{Direct criticism:} ``bad'' (40{,}331), ``boring'', ``disappointing''
    \item \textit{Economic concerns:} ``money'' (28{,}064)
    \item \textit{Temporal qualifiers:} ``after'' (29{,}286), ``then'' (28{,}351)
    \item \textit{Causality markers:} ``because'' (40{,}281)
\end{itemize}
...
\begin{itemize}
    \item \textit{Balanced sentiment terms:} ``good'' (62{,}804), ``fun'' (44{,}575), ``great'' (32{,}117), ``bad'' (26{,}632)
    \item \textit{Comparative language:} ``better'' (24{,}681), ``still'' (27{,}706)
    \item \textit{Qualification markers:} ``feel'', ``way'' (23{,}596)
\end{itemize}
...
Terms such as ``first'', ``still'', and ``after'' suggest longitudinal evaluation, capturing changes in perception over time due to gameplay depth, updates, or post-release support.
...
\subsection{Overall metrics (global)}
...
Table~\ref{tab:per_class_metrics} reports per-class Precision, Recall, and F1 on the test set for all models. Overall, all models perform strongly on \textbf{Positive} and \textbf{Negative} sentiment, while \textbf{Mixed} remains the most challenging class with noticeably lower Precision/Recall/F1. RoBERTa is the most consistent across classes, achieving the best overall balance—especially improving Mixed-class F1—while EmbeddingGemma + XGBoost shows a stronger trade-off between precision and recall depending on the class (notably weaker recall for Mixed). LSTM is competitive on the major classes but struggles most on Mixed, suggesting it benefits less from nuanced or ambiguous examples compared to RoBERTa.
...
Mixed $16.71\%$ predicted vs.\ $16.13\%$ actual ($+0.59$ pp), Negative $26.05\%$ vs.\ $25.63\%$ ($+0.42$ pp), Positive $57.24\%$ vs.\ $58.24\%$ ($-1.01$ pp).
It classifies Positive well, but Mixed is frequently confused with both Negative and Positive, indicating Mixed is the hardest class. The predicted class distribution slightly overestimates Mixed/Negative and underestimates Positive.
...
Mixed $12.46\%$ predicted vs.\ $16.13\%$ actual ($-3.67$ pp), Negative $28.25\%$ vs.\ $25.63\%$ ($+2.62$ pp), Positive $59.29\%$ vs.\ $58.24\%$ ($+1.05$ pp).
It strongly under-predicts Mixed, shifting many Mixed samples into Negative and Positive, and tends to over-predict Negative. Overall, it is conservative on Mixed and biased toward labeling ambiguous cases as Negative.
...
Mixed $15.24\%$ predicted vs.\ $16.13\%$ actual ($-0.89$ pp), Negative $25.81\%$ vs.\ $25.63\%$ ($+0.18$ pp), Positive $58.96\%$ vs.\ $58.24\%$ ($+0.71$ pp). 
It shows the cleanest separation overall, with a stronger diagonal—especially for Mixed—and fewer cross-class confusions than the other models. Its predicted distribution is closest to the true distribution, although Mixed remains the most challenging class.
...
\begin{itemize}
  ...
  \item \textbf{Unreliable aggregate metrics on high-value content.} Sentiment summaries derived from the LSTM become least trustworthy on nuanced, thoughtful reviews—the very content most valuable for product and market insights.
